...
hinemos_stop_monitor() {
    # ステップ①：3つの監視を個別に停止
    _check_hinemos_cli
    log_info "Hinemos 監視停止開始（3件）"
    for monitor_id in "${HINEMOS_MONITOR_IDS[@]}"; do
        log_info "  監視停止: ${monitor_id}"
        # TODO: CLIコマンド構文確定後に有効化
        # "${HINEMOS_CLI_CMD}" monitor stop "${monitor_id}"
    done
...
hinemos_stop_jobs() {
    ...
    for job_id in "${HINEMOS_JOB_IDS[@]}"; do
        ...
        # TODO: CLIコマンド構文確定後に有効化
        # "${HINEMOS_CLI_CMD}" job stop "${HINEMOS_JOBUNIT_ID}" "${job_id}"
    done
...
hinemos_start_jobs() {
    ...
    for job_id in "${HINEMOS_JOB_IDS[@]}"; do
        ...
        # TODO: CLIコマンド構文確定後に有効化
        # "${HINEMOS_CLI_CMD}" job start "${HINEMOS_JOBUNIT_ID}" "${job_id}"
    done
...
hinemos_start_monitor() {
    # ステップ④：3つの監視を個別に再開
    _check_hinemos_cli
    log_info "Hinemos 監視再開開始（3件）"
    for monitor_id in "${HINEMOS_MONITOR_IDS[@]}"; do
        log_info "  監視再開: ${monitor_id}"
        # TODO: CLIコマンド構文確定後に有効化
        # "${HINEMOS_CLI_CMD}" monitor start "${monitor_id}"
    done
